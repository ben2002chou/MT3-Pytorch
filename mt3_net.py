...
pl.seed_everything(365, workers=True)


class MT3Net(pl.LightningModule):

    def __init__(
        self, config, model_config_path="config/mt3_config.json", result_dir="./results"
    ):
        super().__init__()
        self.config = config
        ...
        if self.config.get("pretrained", None) is not None:
            self.model.load_state_dict(
                torch.load(self.config.pretrained, map_location="cpu"), strict=True
            )
    ...
    def train_dataloader(self):
        ...
        dataset = MidiMixIterDataset(
            train_path,
            mel_length=self.config.mel_length,
            event_length=self.config.event_length,
...
def main(config, model_config, result_dir):
    ...
    print(model)
    logger = WandbLogger(project="mt3-pytorch-2")
